...
class Player {
    ...
    CheckCollisions(currentLines) {
        ...
        if (chosenLine.isHorizontal) {
            ...
            if (this.IsMovingRight()) {
                ...
                //storing speed before any transitions made
        ...
        if (collidedLines.length > 1) {
            ...
            //if landed, but last correction might have moved player off surface
            if (potentialLanding) {
    ...
    IsCollidingWithLine(l) {
        if (l.isHorizontal) {
            let isRectWithinLineX = (l.x1 < this.currentPos.x && this.currentPos.x < l.x2) || (l.x1 < this.currentPos.x + this.width && this.currentPos.x + this.width < l.x2) || (this.currentPos.x < l.x1 && l.x1 < this.currentPos.x + this.width) || (this.currentPos.x < l.x2 && l.x2 < this.currentPos.x + this.width);
            let isRectWithinLineY = this.currentPos.y < l.y1 && l.y1 < this.currentPos.y + this.height;
            return isRectWithinLineX && isRectWithinLineY;
        } else if (l.isVertical) {
            let isRectWithinLineY = (l.y1 < this.currentPos.y && this.currentPos.y < l.y2) || (l.y1 < this.currentPos.y + this.height && this.currentPos.y + this.height < l.y2) || (this.currentPos.y < l.y1 && l.y1 < this.currentPos.y + this.height) || (this.currentPos.y < l.y2 && l.y2 < this.currentPos.y + this.height);
            let isRectWithinLineX = this.currentPos.x < l.x1 && l.x1 < this.currentPos.x + this.width;
            return isRectWithinLineX && isRectWithinLineY;
        } else {
    ...
    UpdatePlayerSlide(currentLines) {
        if (this.isSlidding) {
            if (!this.IsPlayerOnDiagonal(currentLines)) {
                this.isSlidding = false; //therefore not sliding
            }
        }
    ...
    IsPlayerOnGround(currentLines) {
        this.currentPos.y += 1;
        for (const i of currentLines) {
            if (i.isHorizontal && this.IsCollidingWithLine(i)){
                this.currentPos.y -= 1;
                return true;
            }     
        }
    ...
    IsPlayerOnDiagonal(currentLines) {
        this.currentPos.y += 5;
        for (const i of currentLines) {
            if (i.isDiagonal && this.IsCollidingWithLine(i)){
                this.currentPos.y -= 5;
                return true;
            }     
        }
            ...
            for (let l of collidedLines) {
                ...
                if (l.isHorizontal) {
                   ...
                   if (l.diagonalCollisionInfo.collisionPoints.length === 2) {
                        ...
                        if (playerCornerPos === null) { //redundancy because errors
                            print(left, right, top, bottom);
                            playerCornerPos = this.currentPos.copy();
    ...
    CheckForLevelChange() {
        if (this.currentPos.y < -this.height) {
            ...
            if (this.currentLevelNo === 0) {
                //player gone under level 1
                this.currentLevelNo = 1; //lol easily fixed
                this.playersDead = true;
                this.hasFinishedInstructions = true;
            }
            this.currentLevelNo -= 1;
    ...
    UpdateAIAction() {
        //runs every frame, for action call
    ...
    playerLanded() {
        // if moving down then landed
        this.isOnGround = true
        // if ice level then slide instead
        if (levels[this.currentLevelNo].isIceLevel) {
            this.currentSpeed.y = 0;
            if (this.IsMovingRight()) {
                this.currentSpeed.x -= iceFrictionAcceleration;
