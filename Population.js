...
class Population {
    ...
    SetBestPlayer() {
        ...
        if (this.players[this.bestPlayerIndex].bestLevelReached > this.currentBestLevelReached) {
            ...
            console.log("NEW LEVEL, action number", this.reachedBestLevelAtActionNo)
        }
    ...
    Show() {
        ...
        for (const i of this.players) {
            if (i.currentLevelNo >= highestLevelNo - 1 && i.currentLevelNo <= highestLevelNo){
                i.Show();
            }
        }
    }
    ...
    IncreasePlayerMoves(increaseBy) {
        for (const i of this.players) {
            i.brain.increaseMoves(increaseBy);   
        }
    }

    AllPlayersFinished() {
        for (const i of this.players) {
            if (!i.hasFinishedInstructions){
                return false;
            }
        }
        return true;
